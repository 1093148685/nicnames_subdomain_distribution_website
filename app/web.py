...
async def get_invite(request: Request, db: Session = Depends(get_db)):
    ...
    return {
        "code": code,
        "link": f"https://REDACTED.example.com/invite/{code}",
        "count": len(records),
        "earnings": earnings,
...
async def check_subdomain(request: Request, db: Session = Depends(get_db)):
    ...
    price = _domain_price(db, root_domain, domain_info.get("credits", 10))
    ...
    if await run_in_threadpool(_nicnames_prefix_taken, db, prefix, root_domain):
        return {"available": False, "price": price, "reason": "NicNames 已存在该前缀记录"}
...
async def register_subdomain(request: Request, db: Session = Depends(get_db)):
    ...
    _sync_nicnames_add(db, sub, "A", "@", "34.104.199.208", 14400)
    user.credits -= price
    db.add(sub)
    db.add(DNSRecord(subdomain=sub, type="A", name="@", content="34.104.199.208", ttl=14400))
    sub.records_count = 1
...
async def admin_dns_records(request: Request, db: Session = Depends(get_db)):
    admin = require_admin(request, db)
    records = db.query(DNSRecord).order_by(DNSRecord.id.desc()).limit(200).all()
    return {"records": [record_to_dict(r) for r in records]}
