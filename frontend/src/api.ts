...
export const api = {
  ...
  adminGetPremiumDomains: () => request<{ domains: any[] }>('/api/admin/premium-domains'),
  adminAddPremiumDomain: (domain: string, price: number) =>
    request('/api/admin/premium-domains', { method: 'POST', body: JSON.stringify({ domain, price }) }),
  adminDeletePremiumDomain: (domain: string) =>
    request(`/api/admin/premium-domains/${encodeURIComponent(domain)}`, { method: 'DELETE' }),
