                  ...
                  {authConfig.oidc_providers?.map((p: any) => p.enabled && (
                    <button key={p.key} type="button" className={`btn oidc-btn oidc-${p.key}`}
                      onClick={() => {
                        ...
                        window.location.href = `/api/auth/oidc/${p.key}?redirect=${encodeURIComponent(current)}`
                      }}>
                      {p.key === 'github' ? GithubIcon : p.key === 'linuxdo' ? LinuxdoIcon : p.key === 'hongconnect' ? HongConnectIcon : null}
                      {p.name}
                    </button>
                  ))}
                </div>
