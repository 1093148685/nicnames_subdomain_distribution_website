...
function RecordsTab() {
  ...
  return (
    <>
      <div className="search-bar">
        <input className="input" placeholder="搜索名称或内容..." value={search}
          ...
          <tbody>
            {records.map(r => (
              <tr key={r.id}>
                ...
                <td style={{ fontFamily: 'monospace', fontSize: '0.8125rem', wordBreak: 'break-all', maxWidth: '200px' }}>
                  {r.content}
                </td>
                <td style={{ fontSize: '0.8125rem' }}>{r.subdomain || r.subdomain_name || '-'}</td>
            ...
            {records.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>暂无记录</td></tr>
            )}
          </tbody>
        </table>
      </div>
