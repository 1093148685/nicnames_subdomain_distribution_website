    ...
    <div>
      ...
      <div className="settings-grid" style={{ marginBottom: '1rem' }}>
        ...
        <section className="card settings-card">
          ...
          <label className="form-group"><span>默认注册 A 记录 IP</span>
            <input className="input" value={quick.default_register_ip || ''} onChange={e => setText('default_register_ip', e.target.value)} placeholder="留空自动获取当前服务器公网 IP" />
          </label>
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginTop: '0.5rem' }}>
            留空时后端会自动获取当前服务器公网 IP，并把检测结果写入 <code>default_register_ip_detected</code>；单个高级根域名可在「高级前缀」页设置固定价格。
          </p>
        </section>
